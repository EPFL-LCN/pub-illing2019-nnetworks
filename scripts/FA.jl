####################################################
# Script to train a deep network with feedback alignment (SGD)
...
dataset = "CIFAR10" # dataset to be used: "MNIST" or "CIFAR10"
...
print(string("train network with 1 hidden layer with ",hidden_size," neurons...\n"))
for j in 1:n_inits
    net = Network_RF_sparseconnect([size(smallimgs)[1], hidden_size, 10])
    set_connectivity_matrix!(net,patch_size; mode = "randn", colorimage = colorimage) # set connectivity of patches
    @time loss = learn!(net, getsmallimg, getlabel, iterations, learningrates, 0.,
